  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (freq) {
      ...
      freq.innerHTML = rows.map(([n, c]) => {
        ...
        return `<tr>
          <td><span class="num-chip">${n}</span></td>
          <td>${c}</td>
          <td><div style="height:8px;border-radius:6px;background:rgba(255,255,255,0.08);overflow:hidden">
            <div style="width:${pct}%;height:100%;background:linear-gradient(90deg,var(--gold),var(--magenta))"></div>
          </div></td>
        </tr>`;
      }).join("");
    }
